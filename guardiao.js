...
const { IdentificarUsuarioDialog } = require('./componentDialogs/identificarUsuarioDialog');
...
class Guardiao extends ActivityHandler {
    ...
    constructor(conversationState, userState) {
        ...
        this.identificarUsuarioDialog = new IdentificarUsuarioDialog(this.conversationState, this.userState);
    ...
    async dispatchToIntentAsync(context, intent, entities) {
        ...
        if (context._activity.channelId !== 'alexa') {
            ...
            if (('attachments' in context.activity) && (context.activity['attachments'][0].contentType !== 'image/jpeg')) {
                ...
                if (conversationData.endDialog) {
                    var identificacao = await this.identificarUsuarioDialog.getIdentificacao();    
                   ...
                   if ((identificacao !== '') && (identificacao !== 'none')) {                       
                       ...
                       await context.sendActivity(await this.translator.t('A imagem não corresponde a nenhum usuário. Tente novamente.'));                  
                   }
        ...
        if ((intent === 'Configurar_Tag_Alerta') || (currentIntent === 'Configurar_Tag_Alerta')) {
            ...
            const usuario = await storage.getItem(context._activity.from.id + 'login');
            if (usuario !== undefined) {
                await context.sendActivity(await this.translator.t(usuario + ' foi desconectado!'));                            
